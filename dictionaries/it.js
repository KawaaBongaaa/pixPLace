export const it = {
    loading: "The pixPLace",
    app_title: "pixPLace",
    connecting: "Connessione...",
    connected: "Connesso a Telegram",
    prompt_label: "Prompt",
    prompt_placeholder: "Descrivi la tua visione e guarda l'IA darle vita...",
    style_label: "Stile",
    style_realistic: "Reale",
    style_artistic: "Artistico",
    style_cartoon: "Cartoon",
    style_fantasy: "Fantasy",
    style_anime: "Anime",
    style_cyberpunk: "Cyberpunk",
    style_popart: "Pop Art",
    style_abstract: "Astratto",
    style_sketch: "Sketch",
    style_3d: "3D",
    style_sticker: "Sticker",
    style_vector: "Vettore",
    style_interior: "Interno",
    style_architecture: "Architettura",
    style_fashion: "Moda",
    style_tattoo: "Tatuaggio",
    style_print: "Print",
    style_logo: "Logo",
    style_minimalism: "style_minimalism",
style_banner: "Banner",
style_infographic: "Infografica",
mode_label: "Modalità",
    mode_background_removal: "Rimozione Sfondo",
    mode_upscale_image: "Migliora Immagine",
    mode_print_maker: "SDXL Stampa Magliette | PoD Helper",
    mode_photo_session: "Nano Banana | Editor Foto",
    mode_fast_generation: "Flux Shnel | Veloce | Immagini semplici",
    mode_pixplace_pro: "Flux Krea | Logo | Testo",
    mode_details: "Dettagli del Modello",

    // Descrizioni delle modalità
    mode_photo_session_desc: "Perfetto per la modifica foto — guida l'AI con fino a 4 riferimenti immagine o lascia che lavori liberamente",
    mode_fast_generation_desc: "Il modello più veloce per generazione rapida di immagini — funziona istantaneamente senza upload",
    mode_pixplace_pro_desc: "Passa alla Modalità Professionale — ideale per design di loghi, composizioni di testo e layout complessi come un vero Designer Pro",
    mode_print_maker_desc: "Progettato appositamente per creatori di Print-on-Demand — crea design pronti per la stampa su abbigliamento, borse e altro",
    mode_background_removal_desc: "Rimuovi lo sfondo e mantieni solo il soggetto principale. Carica una foto per iniziare",
    mode_upscale_image_desc: "Migliora qualità e risoluzione delle immagini — rende i tuoi visual nitidi e dettagliati",

    size_label: "Dimensione",
    size_square: "1:1",
    size_portrait: "9:16",
    size_landscape: "16:9",
    generate_btn: "Genera Immagine",
    create_new: "Crea Nuovo",
    empty_history_title: "Nessuna generazione",
    empty_history_subtitle: "Crea la tua prima immagine AI per vederla qui",
    generation_cost: "Costo Generazione",
    error_prompt_required: "Per favore descrivi la tua immagine",
    error_prompt_too_short: "Descrizione troppo corta (minimo 5 caratteri)",
    error_webhook_not_configured: "Webhook non configurato",
    error_generation_failed: "Generazione fallita",
    error_timeout: "Timeout. Riprova.",
    error_server_overloaded: "😓 Generazione fallita. I server sono attualmente sovraccarichi, riprova più tardi o seleziona una modalità di generazione diversa... Ci scusiamo sinceramente per i disagi e speriamo nella vostra comprensione 🙏",
    success_generated: "Immagine generata con successo!",
    copied_to_clipboard: "Copiato negli appunti",
    download_started: "Download avviato",
    generation_queued: "Generazione aggiunta alla coda",
    generation_started: "Generazione avviata",
    limit_title: "Limite raggiunto",
    limit_message: "I tuoi crediti sono terminati. Puoi ottenerne altri con un abbonamento a pagamento.",
    closeLimitModal: "Forse dopo",
    upgradeBtn: "Paga",
    premium_limit_title: "Il tuo limite premium è esaurito!",
    premium_limit_message: "Continua con le funzioni standard o fai l'upgrade per accesso completo.",
    remove_user_image: "Rimuovi",
    reference_image: "Riferimento",
    upload_image: "Carica Immagine",
    ai_coach_ready: "✨ AI Prompt Helper pronto!",
    ai_welcome_intro: "AI Prompt Helper: Benvenuto! Sono il tuo assistente IA.",
    ai_placeholder_modal: "Scrivi al tuo assistente AI...",
    ai_send_button: "Invia",
    ai_close_button: "Chiudi",
    ai_thinking_indicator: "🤖 pixPLace Assistant sta pensando...",
    ai_error_message: "Errore. Riprova.",
    most_popular: "Più Popolare",
    elapsed_time: "Tempo trascorso:",
    view_history: "Vedi Cronologia",
    history_title: "Cronologia",
    ai_chat_title: "Assistente IA",
    plan_lite_title: "LITE",
    plan_lite_price: "€12",
    plan_lite_select: "Abbonati",
    plan_pro_title: "PRO",
    plan_pro_price: "€17",
    plan_pro_select: "Abbonati",
    plan_studio_title: "STUDIO",
    plan_studio_price: "€31",
    plan_studio_select: "Abbonati",
    photo_warning_title: "Per risultati migliori, carica un'immagine",
    photo_warning_text: "La modalità \"Nano Banana\" funziona meglio con un'immagine per la generazione img2img. Vuoi caricare un'immagine o continuare senza?",
    photo_warning_upload_btn: "Carica immagine",
    photo_warning_continue_btn: "Continua senza",
    image_limit_error: "Massimo 4 immagini. Puoi caricarne {{count}} in più.",
    load_more_history: "Mostra di più"
}
