...
const TRANSLATIONS = {
    ...
    ru: {
        ...
        mode_pixplace_pro: "Pro | Logo | Text ~5cr",
        size_label: "Размер",
        size_square: "1:1",
        size_portrait: "9:16",
        size_landscape: "16:9",
        ...
        limit_message: "Ваши кредиты закончились.\nБольше кредитов Вы можете получить, оформив один из тарифных планов.\n\nОплатить можно с любой карты или криптой, используя платежный сервис Tribute Telegram.",
        ...
        please_upload_photo_session: "для Режима Фотосессии необходимо Загрузить фото с Лицом! Или выбери другой Режим Генерации",
...
// ------- viewport / --vh helpers -------
function setVhFromWindowOrTelegram() {
    try {
        const tg = window.Telegram?.WebApp;
        // Используем Telegram.viewportHeight когда есть (точнее внутри WebApp)
        const height = (tg && typeof tg.viewportHeight === 'number' && tg.viewportHeight > 0)
            ? tg.viewportHeight
            : window.innerHeight;
...
let _tgViewportPollId = null;
function startTelegramViewportPoll(interval = 300) {
    if (_tgViewportPollId) return;
    let last = window.Telegram?.WebApp?.viewportHeight || 0;
    _tgViewportPollId = setInterval(() => {
        const cur = window.Telegram?.WebApp?.viewportHeight || 0;
        if (cur && cur !== last) {
            last = cur;
            setVhFromWindowOrTelegram();
        }
    }, interval);
}
function stopTelegramViewportPoll() {
    if (_tgViewportPollId) {
        clearInterval(_tgViewportPollId);
        _tgViewportPollId = null;
    }
}
// ------- /viewport helpers -------
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    try {
        ...
        setVhFromWindowOrTelegram();
        startTelegramViewportPoll();
        window.addEventListener('resize', setVhFromWindowOrTelegram);    // 👉 только теперь можно обращаться к WebApp
    } catch (e) {
